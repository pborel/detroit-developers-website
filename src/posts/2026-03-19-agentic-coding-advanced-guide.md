...
## [DORA AI Capabilities Model](https://services.google.com/fh/files/misc/2025_dora_ai_capabilities_model.pdf) (Google, 2025)
...
Fresh data from DX (presented by Laura Tacho at [The Pragmatic Summit](https://newsletter.pragmaticengineer.com/p/the-future-of-software-engineering-with-ai) in February 2026) puts numbers on this: 92% of developers now use AI coding assistants at least monthly, with self-reported savings averaging ~4 hours per week. But the outcomes are diverging sharply. Organizations with healthy engineering systems are seeing 50% fewer customer-facing incidents. Dysfunctional organizations are seeing twice as many. As Tacho put it: "AI is an accelerator, it's a multiplier, and it is moving organizations in different directions."

## [Building an Elite AI Engineering Culture](https://www.cjroth.com/blog/2026-02-18-building-an-elite-engineering-culture) (CJ Roth, February 2026)
...
## [Harness Engineering](https://openai.com/index/harness-engineering/) (OpenAI, February 2026)
...
## [The Cautionary Data](https://newsletter.pragmaticengineer.com/p/are-ai-agents-actually-slowing-us) (Pragmatic Engineer, March 2026)

The research above tells you what to do. This piece tells you what happens when you don't.

Gergely Orosz's reporting in [The Pragmatic Engineer](https://newsletter.pragmaticengineer.com/p/are-ai-agents-actually-slowing-us) synthesizes a growing body of evidence that AI agents - adopted without discipline - are actively degrading software quality and, in some cases, slowing teams down.

The alarming data comes from a [Carnegie Mellon study](https://arxiv.org/abs/2511.04427) of 807 open-source repositories that adopted Cursor. The findings are sobering: projects saw a roughly 281% increase in lines added during the first month. By month two, velocity gains had disappeared entirely and code activity was back to pre-adoption levels. Meanwhile, static analysis warnings rose ~30% and code complexity increased ~41% - permanently. The researchers found a self-reinforcing negative cycle: accumulated tech debt from AI-generated code significantly reduced future development velocity. Doubling code complexity was associated with a ~65% drop in new lines added going forward.
...
**PR review becomes a traffic jam.** Per the [Faros AI data](https://www.cjroth.com/blog/2026-02-18-building-an-elite-engineering-culture) (cited in Roth's post above - their study of 10,000+ developers across 1,255 teams): 98% more PRs merged, 91% more review time. If we don't address review tooling and PR size, we'll create a pile-up.

**PR size is a real policy question.** Agentic coding makes it easy to produce work in large batches. Large PRs are slow to review & high risk to merge (the _area of impact_ is large). The [DORA research](https://services.google.com/fh/files/misc/2025_dora_ai_capabilities_model.pdf) identifies working in small batches as one of seven foundational capabilities for AI-amplified performance. The [Carnegie Mellon study](https://arxiv.org/abs/2511.04427) quantifies the cost of ignoring this: the code complexity generated by large AI-assisted changes was associated with a ~65% drop in future development velocity. Working in small batches is not just a nice habit - it's a prerequisite for getting the benefits of AI-accelerated development without the destabilization.

## AI is an Amplifier of More Than Output

This is the thread that connects the [DORA research](https://services.google.com/fh/files/misc/2025_dora_ai_capabilities_model.pdf), the [Roth framework](https://www.cjroth.com/blog/2026-02-18-building-an-elite-engineering-culture), and our own experience. AI amplifies whatever's already there:
...
The [70% problem](https://addyo.substack.com/p/the-70-problem-hard-truths-about) is real when you're building software that needs to handle edge cases, survive production traffic, and be maintained by a team over years. But there's a large category of work where 70% is 100% of what you need. Prototypes. Internal tools. Simple sites. Things that don't require scale, infinite maintainability or that you're going to throw away. [Osmani](https://addyo.substack.com/p/the-70-problem-hard-truths-about) nails this distinction: AI tools are exceptional as "prototyping accelerators" and "MVP generators for validating ideas quickly." The problem only arises when people mistake the prototype for the product.
...
We're not tracking full DORA metrics yet. Right now, our leading indicators are PRs per week, projects per quarter, points per sprint, test coverage and Change Failure Rate. We also track support ticket count - we're closing them significantly faster now, which shows up as a lower backlog maintained per two-week support shift. These aren't optimal measures, but they're a starting place. We used the [DORA AI Report's](https://services.google.com/fh/files/misc/2025_dora_ai_capabilities_model.pdf) cluster analysis to help identify where to start improving our systems to get the most out of agentic coding, and we plan to move toward proper DORA metrics over time.
...
**Hooks** trigger automated actions based on agent behavior - for example, running linters after code generation, or auto-formatting files. We haven't fully leveraged hooks yet, but they're on the roadmap as the next layer of automation. Combined with rules, hooks are how you build the kind of continuous mechanical enforcement that the [OpenAI team](https://openai.com/index/harness-engineering/) credits as a key multiplier - constraints that apply everywhere at once, without human attention.
